...
function processQuestionaireForPDF(
	...
	const questionaireAsList: List = { indexCharacter: "num", type: "ordered", items: [] };

	const answerAsList: List = { indexCharacter: "num", type: "ordered", items: [] };
	for (const item of questionaire) {
		...
		} else if (item.typeOfAnswer === "BOOLEAN" || item.typeOfAnswer === "ESSAY") {
			questionaireAsList.items.push(item.question);
		}
		...
		if (item.typeOfAnswer !== "ESSAY") {
			if (item.typeOfAnswer === "BOOLEAN") {
				...
				answerAsList.items.push(
					item.answers.map((answer) => String.fromCharCode("A".charCodeAt(0) + answer)).toString()
				);
			}
		}
	}
...
function processSummaryForPDF(pdfGen: TopDownPDFGenerator, summary: string): TopDownPDFGenerator {
	pdfGen.setFontConfig({ fontSizeInPt: 12, fontEmphasis: "normal" });
	if (summary.includes("*")) {
		pdfGen.appendList({
			type: "unordered",

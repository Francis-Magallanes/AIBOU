...
class TopDownPDFGenerator {
  ...
  constructor(config: TopDownPDFConfig) {
    ...
    this.pdf = new jsPDF({
      format:
        typeof config.format === "object"
          ? Object.values(config.format)
          : config.format,
  ...
  public appendText(
    ...
    const resultingBlockTextHeight =
      (this.pdf.splitTextToSize(text, allowableWritingSpaceWidth) as any[])
        .length * lineHeight;
  ...
  private processOrderedList(
    ...
    for (let i = 0; i < list.items.length; i++) {
      ...
      if (typeof item === "string") {
        ...
        const lineHeight =
          this.pdf.getLineHeight() / this.pdf.internal.scaleFactor;
        const resultingBlockTextHeight =
          (
            this.pdf.splitTextToSize(
              itemWithIndex,
              allowableWritingSpaceWidth,
            ) as any[]
          ).length * lineHeight;
        ...
        if (item.type === "ordered") {
          this.processOrderedList(
            item,
            xPos + this.indentationSizeInInches,
            alignment,
          );

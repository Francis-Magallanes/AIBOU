...
import { CreateQuestionaireConfig, SummarizeConfig } from "../ChooseService/ChooseService";
...
async function processFile(
	...
	if (serviceConfig.typeOfService == "CREATEQUESTIONAIRE") {
		...
		console.error(`The feature "${serviceConfig.typeOfService}" is not yet implemented`);
	}
}
		...
		if (pageTextContent.items.length > 0) {
			textPerPage.push(
				pageTextContent.items
					...
					.map((value) => (value as pdfjsAPI.TextItem).str.replace(/[^\x00-\x7F]/g, "") ?? " ")
					.join("")
			);
		} else {
...
async function processContentForCreateQuestionaire(
	...
	const responseGPT = await gptAPIPostRequestHandler(payloads[payloads.length - 1], abortSignal);
	...
	for (let i = 0; i < splittedQuestionairePortion.length; i++) {
		...
		if (line.match(/^\d+\./g)) {
			...
			if (config.typeOfAnswer !== "ESSAY" && config.typeOfAnswer !== "BOOLEAN") {
...
async function processContentForSummary(
	...
	return await gptAPIPostRequestHandler(payloads[payloads.length - 1], abortSignal);
}

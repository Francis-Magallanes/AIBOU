...
import { useRef, FC, useState, Dispatch, SetStateAction, ChangeEvent, DragEvent } from "react";
...
const DocumentUpload: FC<DocumentUploadProps> = ({
	...
	const onDropFile = (event: DragEvent): void => {
		...
		if (!validFileTypes.some((item) => files[0].name.endsWith(item))) {
			setModalInfo({
				title: "Invalid File",
				content: "The system can only process file that ends with " + validFileTypes.toString(),
			});
			onOpenModal();
			return;
		}
		...
		<>
			<VStack alignItems="normal">
				<Box>
					...
					>
						<Icon as={AiOutlineCloudUpload} boxSize="5rem" />
				...
				<Text hidden={fileName === ""}>
					...
					<Icon as={AiFillCheckCircle} color="green.300" boxSize="1rem" />
				</Text>
			</VStack>
